...
def parse_weapons():
    ...
    for item in weapon_data.get_list():
        if item['id'] in IGNORED_WEAPONS:
            continue
        ...
        resultItem = {
            ...
            'rarity': rarity,
            'gameId': item['id'],
            'weapon': static.WEAPON_TYPES[item['weaponType']],
            'stats': [],
            'refines': [],
        }
        values = {}
        ...
        for id in item['skillAffix']:
            ...
            valList = []
            for i in affixList:
                if convert_id(lang['eng'].get(affixList[0]['nameTextMapHash']))=='bane_of_fire_and_thunder':print(i)
                valList.append(i['paramList'])
            elem = {
                'skill': affixList[0]['openConfig'],
                # 'skill': convert_id(lang['eng'].get(affixList[0]['nameTextMapHash'])),
                'params': []
            }
