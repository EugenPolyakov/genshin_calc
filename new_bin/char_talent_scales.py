...
from lib.genshin.datafiles.char import CharData, CharTalentSkillData, CharSkillDepotData, CharProudSkillData
...
def parse_char_skills(charName, scales, skills, depotId, groupmap, item):
    skillsIds = skills.get(depotId)

    out.write("\t" + charName + ': {\n')
    out.write(f"\t\tchar_id:'{item['id']}',\n")
    out.write(f"\t\tchar_weapon:'{WEAPON_TYPES[item['weaponType']]}',\n")
    ...
    depot = depot_data.get(depotId)
    out.write('\t\tcons: [\n')
    for cons_id in depot.get('talents'):
        talent = talent_data.get(cons_id)
        if not talent:
            continue
        out.write('\t\t\t')
        out.write(repr(remove_zeros(talent.get('paramList'))))
        out.write(',\n')
    out.write('\t\t],\n')

    out.write('\t\tpasssive: [\n')
    for passive in depot.get('inherentProudSkillOpens', []):
        if passive.get('proudSkillGroupId'):
            passive_id = passive.get('proudSkillGroupId')
            proud = proud_data.get_item_by_field('proudSkillGroupId', passive_id)
            out.write('\t\t\t')
            out.write(repr(remove_zeros(proud.get('paramList'))))
            out.write(',\n')
    out.write('\t\t],\n')
    out.write('\t},\n')
    ...
    for charName in sorted(chars.keys()):
        ...
        if len(depotIds) > 1:
            for depotId in depotIds:
                ...
                if not charName:
                    continue
                parse_char_skills(charName, scales, skills, depotId, groupmap, item)
        else:
            parse_char_skills(charName, scales, skills, item.get('skillDepotId'), groupmap, item)
