...
from lib.genshin.datafiles.char import AvatarCurveExcelConfigData, CharData, CharProudSkillData, CharSkillData, CharSkillDepotData, SKIP_CHARACTERS, CharTalentSkillData
...
talenttable('function FixTable(arr, isPercent) {\n')
talenttable('\tif(isPercent)\n')
talenttable('\t\treturn arr.map(x => Math.fround(x) * 100);\n')
talenttable('\telse\n')
talenttable('\t\treturn arr.map(x => Math.fround(x));\n')
talenttable('}\n')
...
def parse_curves():
    file = AvatarCurveExcelConfigData()
